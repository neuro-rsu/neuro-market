...
export default class DataSet {
    ...
    static #fetchGetItems() {
        return fetch(`https://${HOST}:${PORT}/api/city-types`)
    }
    ...
    static fetchAddItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/city-type`, {
            method: "POST",
            headers: {
    ...
    static #fetchGetItem(itemId) {
        return fetch(`https://${HOST}:${PORT}/api/city-type/${itemId}`)
    }
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/city-type/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/city-type/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
