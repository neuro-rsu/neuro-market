...
export default class DataSet {
    ...
    static #fetchGetItems(token, id) {
        return fetch(`https://${HOST}:${PORT}/api/competition-referees/${id}`, {
            headers: {
    ...
    static fetchAddItem(token, item, id) {
        return fetch(`https://${HOST}:${PORT}/api/competition-referee/${id}`, {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`,
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/competition-referee/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/competition-referee/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
