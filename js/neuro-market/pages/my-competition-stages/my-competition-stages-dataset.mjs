...
export default class DataSet {
    ...
    static #fetchGetItems() {
        return fetch(`https://${HOST}:${PORT}/api/competition-stages`)
    }
    ...
    static fetchAddItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/competition-stage`, {
            method: "POST",
            headers: {
    ...
    static #fetchGetItem(itemId) {
        return fetch(`https://${HOST}:${PORT}/api/competition-stage/${itemId}`)
    }
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/competition-stage/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/competition-stage/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
