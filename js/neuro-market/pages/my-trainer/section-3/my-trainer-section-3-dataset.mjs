...
export default class DataSet {
    ...
    static fetchAddItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/trainer-request`, {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`,
    ...
    static #fetchGetItem(token, itemId) {
        return fetch(`https://${HOST}:${PORT}/api/trainer-request/${itemId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/trainer-request/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/trainer-request/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
