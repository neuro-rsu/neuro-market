...
class MyTrainerSection1 extends BaseElement {
        ...
        raw_data.forEach((r, index) => {
            if(index !== 0){
                const newItem = {
                    ...
                    category: {
                        "_id": "trainer-category:01J7NQ2NX0G3Y1R4D0GY1FFJT1",
    ...
    get page1() {
        return html`
            <my-trainer-section-1-page-1 .oldValues=${this.oldValues} .item=${this.currentItem}></my-trainer-section-1-page-1>
        `;
    }

    get page2() {
        return html`
            <my-trainer-section-1-page-2 .oldValues=${this.oldValues} item=${this.currentItem}></my-trainer-section-1-page-2>
        `;
    }
    ...
    #list1() {
        return html`
            <div class="avatar">
                ${this.isFirst ? html`<avatar-input id="avatar" .currentObject=${this} .avatar=${this.avatar || (this.currentItem.gender == true ? 'images/trainer-woman-solid.svg' : 'images/trainer-man-solid.svg')} @input=${this.validateAvatar}></avatar-input>` : ''}
            </div>
            <div class="label">
    ...
    #list2() {
        return html`
            <my-trainer-section-1-list-2 .parent=${this.currentItem}></my-trainer-section-1-list-2>
        `;
    }
    ...
    async getQRCode() {
        const dataURI = await DataSet.getQRCode(location.origin+`/system?trainer=${this.currentItem._id.split(':')[1]}#my-trainer`)
...
customElements.define("my-trainer-section-1", MyTrainerSection1);