...
export default class DataSet {
    ...
    static #fetchGetItems(token, id) {
        return fetch(`https://${HOST}:${PORT}/api/sportsmen/by-trainer/${id}`, {
            headers: {
    ...
    static fetchAddItem(token, item, id) {
        return fetch(`https://${HOST}:${PORT}/api/trainer-sportsman/${id}`, {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`,
    ...
    static #fetchGetItem(itemId) {
        return fetch(`https://${HOST}:${PORT}/api/trainer-sportsman/${itemId}`)
    }
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/trainer-sportsman/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/trainer-sportsman/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
