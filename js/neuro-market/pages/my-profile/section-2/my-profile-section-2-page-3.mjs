...
class MyProfileSection2Page3 extends BaseElement {
    ...
    render() {
        return html`
            <div class="container">
                ...
                <simple-select id="category" label="${lang`Category`}:" icon-name="referee-category-solid" @icon-click=${() => this.showPage('my-referee-categories')} .dataSource=${this.refereeCategoryDataSource} .value=${this.item?.payload?.category} .currentObject=${this.item?.payload} @input=${this.validateInput}></simple-select>
                ...
                ${this.item?.payload?.refereePC ? html`
                    <simple-input id="refereePC" label="${lang`Referee PC`}:" icon-name="referee-pc-solid" @icon-click=${this.copyToClipboard} .value=${this.item?.payload?.refereePC} .currentObject=${this.item?.payload}></simple-input>
                ` : ''}
                ...
                ${this.item?.payload?.link ? html`
                    <simple-input id="link" label="${lang`Person link`}:" icon-name="user-link" @icon-click=${this.linkClick} .value=${this.item?.payload?.link} .currentObject={this.item?.payload?.order}></simple-input>
                `: ''}
            </div>
        `;
    }
...
customElements.define("my-profile-section-2-page-3", MyProfileSection2Page3);