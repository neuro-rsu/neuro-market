    ...
    render() {
        return html`
            <div class="container">
                ...
                <simple-select id="position" label="${lang`Position`}:" icon-name="federation-member-position-solid" @icon-click=${() => this.showPage('my-federation-member-positions')} .dataSource=${this.federationMemberCategoryDataSource} .value=${this.item?.payload?.position} .currentObject=${this.item?.payload} @input=${this.validateInput}></simple-select>
                ...
                ${this.item?.payload?.federationMemberPC ? html`
                    <simple-input id="federationMemberPC" label="${lang`Federation member PC`}:" icon-name="federation-member-pc-solid" @icon-click=${this.copyToClipboard} .value=${this.item?.payload?.federationMemberPC} .currentObject=${this.item?.payload}></simple-input>
                ` : ''}
...
customElements.define("my-profile-section-2-page-5", MyProfileSection2Page5);