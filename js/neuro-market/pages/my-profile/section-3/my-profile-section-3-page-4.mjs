...
class MyProfileSection3Page4 extends BaseElement {
    ...
    render() {
        return html`
            <div class="container">
                <simple-input label="${lang`Birth certificate number`}:" id="number" icon-name="number-circle-solid" .currentObject=${this.item?.payload} .value=${this.item?.payload?.number} @input=${this.validateInput}></simple-input>
                <simple-input type="date" label="${lang`Birth certificate date`}:" id="date" icon-name="calendar-solid" .currentObject=${this.item?.payload} .value=${this.item?.payload?.date} @input=${this.validateInput} lang="ru-Ru"></simple-input>
    ...
    async downloadFile(){
        this.avatar = await DataSet.downloadAvatar("birth");
