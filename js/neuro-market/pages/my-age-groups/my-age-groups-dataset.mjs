...
export default class DataSet {
    ...
    static #fetchGetItems() {
        return fetch(`https://${HOST}:${PORT}/api/age-groups`)
    }
    ...
    static fetchAddItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/age-group`, {
            method: "POST",
            headers: {
    ...
    static #fetchGetItem(itemId) {
        return fetch(`https://${HOST}:${PORT}/api/age-group/${itemId}`)
    }
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/age-group/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://${HOST}:${PORT}/api/age-group/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
